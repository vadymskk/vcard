...
class vCard implements Countable, Iterator
{
	...
	$enc = detect_utf_encoding($this -> RawData);
	if ($enc) $this -> RawData = iconv($enc, 'utf-8', $this -> RawData);
		}
		elseif ($RawData)
		{
		...
		else
		{
			...
			foreach ($Lines as $Line)
			{
				...
				$Key = mb_strtolower(trim(self::Unescape($Key)));
				...
				if ((mb_strpos($Key, 'agent') === 0) && (mb_stripos($Value, 'begin:vcard') !== false))
				{
					$ClassName = get_class($this);
	...
	public function __get($Key)
	{
		...
		if (isset($this -> Data[$Key]))
		{
			...
			elseif (in_array($Key, self::$Spec_FileElements))
			{
				...
				{
					if (mb_stripos($V['Value'], 'uri:') === 0)
					{
						$Value[$K]['Value'] = mb_substr($V, 4);
	...
	public function SaveFile($Key, $Index = 0, $TargetPath = '')
	{
		...
		if (mb_stripos($this -> Data[$Key][$Index]['Value'], 'uri:') === 0)
		{
			return false;
		}
	...
	private static function ParseParameters($Key, array $RawParams = null)
	{
		...
		{
			$Parameters[] = explode('=', mb_strtolower($Item));
		}
			...
			else
			{
				switch ($Parameter[0])
				{
					...
					case 'type':
						...
						break;
					case 'value':
						if (mb_strtolower($Parameter[1]) == 'url')
						{
							$Result['encoding'] = 'uri';
						}
						break;
